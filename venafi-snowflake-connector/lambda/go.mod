module github.com/starschema/snowflake-venafi-connector

go 1.16

require (
	github.com/Venafi/vcert/v4 v4.14.3 // indirect
	github.com/aws/aws-lambda-go v1.24.0 // indirect
	github.com/aws/aws-sdk-go v1.38.62 // indirect
	github.com/palette-software/go-log-targets v0.0.0-20200609204140-16fbfda0867a // indirect
	github.com/starschema/snowflake-venafi-connector/lambda/utils v0.0.0-00010101000000-000000000000 // indirect
	github.com/urfave/cli v1.21.0 // indirect
	github.com/zfjagann/golang-ring v0.0.0-20210116075443-7c86fdb43134 // indirect

)

replace github.com/starschema/snowflake-venafi-connector/lambda/utils => ./utils
